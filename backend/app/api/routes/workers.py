...
async def register(data: WorkerRegister, session: AsyncSession = Depends(get_db), _: User = Depends(require_role("ADMIN", "OPERATOR"))) -> WorkerResponse: return await register_worker(session, data)
@router.post("/{worker_name}/heartbeat", response_model=WorkerResponse)
async def heartbeat(worker_name: str, session: AsyncSession = Depends(get_db), _: User = Depends(require_role("ADMIN", "OPERATOR"))) -> WorkerResponse:
...
async def workers(session: AsyncSession = Depends(get_db), _: User = Depends(current_user)) -> list[WorkerResponse]: return await list_workers(session)
